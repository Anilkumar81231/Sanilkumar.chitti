...
// Strip Git conflict markers and common junk before JSON.parse
function sanitizeJson(text = "") {
  // remove conflict blocks/markers
  let s = text
    .replace(/^<<<<<<<.*$/gm, "")
    .replace(/^>>>>>>>.*$/gm, "")
    .replace(/^=======\s*$/gm, "");
...
function loadPortfolioForSmalltalk() {
  ...
  const parsed = safeParseJSON(cleaned, fallback);
  ...
  return { ...parsed, owner: { ...(parsed?.owner || {}), name: ownerName } };
}

const portfolio = loadPortfolioForSmalltalk();
const bossName = portfolio?.owner?.name || "Anil Kumar";
const bossDisplay = `my boss ${bossName}`;
...
function replyFor(/* intent */) {
  const base =
    `Oh, me? I'm chitti! ✨ I'm the friendly, witty assistant to ${bossDisplay}. ` +
    `I make sure ${bossName}'s projects and skills get the spotlight they deserve.`;

  const pivot =
    `So, instead of my glamorous AI life 😄, how about we explore ${bossName} — ` +
    `his projects, experience, or latest AI work? What do you want to know about my boss today?`;
